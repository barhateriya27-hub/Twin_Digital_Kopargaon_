        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:block text-right">
              <span className="text-sm font-bold text-slate-900 block">{citizenUser?.name || 'Resident Citizen'}</span>
              <span className="text-[11px] text-emerald-600 font-semibold block">● Account Active</span>
            </div>
            <button
              onClick={handleLogout}
              className="p-2.5 rounded-xl border border-slate-200 text-slate-600 hover:text-rose-600 hover:border-rose-200 bg-white hover:bg-rose-50 transition-all flex items-center gap-1.5 text-xs font-semibold"
              title="Logout"
            >
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              >
                ...
                <p className="text-xs text-slate-500">{userComplaints.length} Total grievances submitted</p>
              </div>
              ...
              >
                ...
                <p className="text-xs text-slate-500">{cityAlerts.length} Active municipal announcements</p>
              </div>
              ...
              >
                ...
                <p className="text-xs text-slate-500">Manage ward info & notification preferences</p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/90 shadow-sm">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="text-xl font-bold text-slate-900">Recent Grievance Submissions</h2>
                  <p className="text-xs text-slate-500 mt-0.5">Live tracking updates from Kopargaon Municipal Control Room</p>
                </div>
                <button
                  onClick={() => setActiveTab('track')}
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between sm:justify-end gap-3 border-t sm:border-t-0 pt-2 sm:pt-0 border-slate-200">
                      <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                        ...
                        item.status === 'Assigned' ? 'bg-purple-100 text-purple-700' : 'bg-amber-100 text-amber-700'
                      }`}>
                        {item.status}
                      </span>
          ...
          <div className="max-w-3xl mx-auto">
            <div className="bg-white rounded-3xl p-8 border border-slate-200/90 shadow-lg shadow-slate-200/40">
              <div className="flex items-center gap-3 mb-6 pb-6 border-b border-slate-100">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-slate-900">Report City Complaint</h2>
                  <p className="text-xs text-slate-500">AI auto-classifies your report and routes it to the designated officer</p>
                </div>
              </div>
                  ...
                  <div className="border-2 border-dashed border-slate-200 rounded-2xl p-6 text-center hover:border-sky-400 transition-colors bg-slate-50/50">
                    {imagePreview ? (
                      <div className="relative inline-block">
                        <img src={imagePreview} alt="Upload preview" className="h-40 rounded-xl object-cover border border-slate-200" />
                        <button
                          type="button"
        ...
        {activeTab === 'track' && (
          <div className="space-y-8">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-bold text-slate-900">Grievance Tracking & History</h2>
                <p className="text-xs text-slate-500">Live status synchronization with Kopargaon Control Room</p>
              </div>
              <button
                onClick={() => setActiveTab('report')}
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold ${
                        ...
                        c.status === 'Assigned' ? 'bg-purple-100 text-purple-700' : 'bg-amber-100 text-amber-700'
                      }`}>
                        {c.status}
                      </span>
                    </div>
              ...
              <div className="lg:col-span-2">
                {selectedComplaint ? (
                  <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/90 shadow-sm">
                    <div className="flex flex-wrap items-center justify-between gap-4 mb-6 pb-6 border-b border-slate-100">
                      ...
                      <span className={`px-4 py-1.5 rounded-full text-xs font-bold ${
                        ...
                        selectedComplaint.status === 'Assigned' ? 'bg-purple-100 text-purple-700' : 'bg-amber-100 text-amber-700'
                      }`}>
                    ...
                    <div className="mb-8 bg-slate-50 p-6 rounded-2xl border border-slate-200/80">
                      <h4 className="text-xs font-semibold text-slate-700 uppercase tracking-wider mb-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                      <div>
                        <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Photo Evidence</h4>
                        <img
                      ...
                      <div className="space-y-4">
                        ...
                        <div>
                          <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Department Assigned</h4>
                          <p className="text-sm font-bold text-slate-800">{selectedComplaint.department}</p>
                        </div>

                        <div>
                          <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Assigned Maintenance Officer</h4>
                        ...
                        <div>
                          <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">AI Classification Confidence</h4>
                        ...
                        {selectedComplaint.timeline?.map((t, i) => (
                          <div key={i} className="p-3.5 bg-slate-50 rounded-xl border border-slate-100 flex items-start gap-3 text-xs">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-slate-600">{t.note}</p>
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  <div className="bg-white rounded-3xl p-12 text-center border border-slate-200 text-slate-400">
                    ...
                    <p className="font-bold text-slate-600">Select a complaint from the list to view live tracking details.</p>
                  </div>
                )}
              </div>
            </div>
        ...
        {activeTab === 'alerts' && (
          <div className="max-w-4xl mx-auto space-y-6">
            <div>
              <h2 className="text-2xl font-bold text-slate-900">Municipal City Alerts</h2>
              <p className="text-xs text-slate-500">Official notifications from Kopargaon Municipal Corporation Control Room</p>
            </div>
          ...
          <div className="max-w-2xl mx-auto">
            <div className="bg-white rounded-3xl p-8 border border-slate-200/90 shadow-sm">
              <div className="flex items-center gap-4 mb-8 pb-6 border-b border-slate-100">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-slate-900">{citizenUser?.name || 'Resident Citizen'}</h2>
              ...
              <div className="space-y-4 text-sm">
                <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                  ...
                  <span className="font-bold text-slate-800">{citizenUser?.email || 'citizen@kopargaon.gov.in'}</span>
                </div>
                <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                  <span className="text-xs text-slate-400 block uppercase font-mono mb-0.5">Mobile Number</span>
                  <span className="font-bold text-slate-800">{citizenUser?.phone || '+91 98220 12345'}</span>
                </div>
                <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                  <span className="text-xs text-slate-400 block uppercase font-mono mb-0.5">Assigned Ward</span>
                  <span className="font-bold text-slate-800">Ward {citizenUser?.ward || 4} (Kopargaon North)</span>
                </div>
                <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                  <span className="text-xs text-slate-400 block uppercase font-mono mb-0.5">Registered Address</span>
                  <span className="font-bold text-slate-800">{citizenUser?.address || 'Sai Nagar, Kopargaon'}</span>
                </div>
              </div>
            </div>
          </div>
