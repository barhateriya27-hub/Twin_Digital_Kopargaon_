    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
              {t('announcements.bannerTitle', 'Public Announcement Management')}
            </h2>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              {t('announcements.bannerSubtitle', 'Broadcast municipal advisories, road closures, water shutdowns, and emergency alerts')}
            </p>
          </div>
        </div>
        ...
        >
          ...
          <span>{isCreating ? t('announcements.btnCloseForm', 'Close Form') : t('announcements.btnPublishNew', 'Publish New Announcement')}</span>
        </button>
      </div>
          ...
          >
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
              <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
                ...
                <span>{editingId ? t('announcements.editTitle', 'Edit Announcement') : t('announcements.composeTitle', 'Compose Public Announcement')}</span>
              </h3>
              <button
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="sm:col-span-2">
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                    {t('announcements.labelTitle', 'Announcement Title *')}
                  </label>
                  <input
                    type="text"
                    required
                    ...
                    placeholder={t('announcements.placeholderTitle', 'e.g. Scheduled Water Supply Shutdown - Ward 2 & Ward 4')}
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                    {t('announcements.labelCategory', 'Category')}
                  </label>
                  <select
                    value={category}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                  {t('announcements.labelDescription', 'Full Advisory Description *')}
                </label>
                <textarea
                  required
                  rows={3}
                  ...
                  placeholder={t('announcements.placeholderDesc', 'Detail the timings, affected locations, alternative routes, safety instructions, or contact helplines...')}
                  className="w-full px-3 py-2 text-xs bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-sky-500"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                    {t('announcements.labelPriority', 'Priority Level')}
                  </label>
                  <select
                    value={priority}
                  ...
                  >
                    <option value="Normal">{t('announcements.prioNormal', 'Normal (Standard Bulletin)')}</option>
                    <option value="High">{t('announcements.prioHigh', 'High (High Priority Notice)')}</option>
                    <option value="Urgent/Emergency">{t('announcements.prioUrgent', 'Urgent/Emergency (Dashboard Red Banner)')}</option>
                  </select>
                </div>

                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                    {t('announcements.labelTargetWards', 'Target Wards')}
                  </label>
                  <input
                    type="text"
                    value={targetWards}
                    onChange={(e) => setTargetWards(e.target.value)}
                    placeholder={t('announcements.placeholderWards', "'All' or comma separated e.g. 1, 2, 4")}
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                    {t('announcements.labelExpiry', 'Expiry Duration')}
                  </label>
                  <select
                    value={expiryDays}
                    ...
                    className="w-full px-3 py-2 text-xs bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-sky-500"
                  >
                    <option value="1">1 {t('announcements.day', 'Day')}</option>
                    <option value="3">3 {t('announcements.days', 'Days')}</option>
                    <option value="7">7 {t('announcements.days', 'Days')}</option>
                    <option value="30">30 {t('announcements.days', 'Days')}</option>
                  </select>
                </div>
              </div>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                >
                  ...
                  <span>{editingId ? t('announcements.btnUpdate', 'Update Announcement') : t('announcements.btnPublishNow', 'Publish Announcement Now')}</span>
                </button>
              </div>
            </form>
          </motion.div>
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
        ...
        >
          {t('announcements.allAnnouncements', 'All Announcements')} ({announcements.length})
        </button>
        {categories.map(cat => (
          <button
            key={cat}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filteredAnnouncements.length === 0 ? (
          <div className="md:col-span-2 p-12 text-center bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 text-slate-400">
            ...
            <p className="text-xs font-medium">{t('announcements.noAnnouncements', 'No published announcements in this category')}</p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs">
                  <div className="text-[11px] text-slate-500 dark:text-slate-400 font-mono">
                    Wards: <span className="font-semibold text-slate-700 dark:text-slate-300">{item.targetWards?.length === 8 ? t('incidentArchive.allWards', 'All Wards') : item.targetWards?.join(', ')}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      title={t('announcements.titleEditNotice', 'Edit Notice')}
                    >
                    ...
                    {!isArchived && (
                      <button
                        ...
                        title={t('announcements.titleArchiveNotice', 'Archive Notice')}
                      >
