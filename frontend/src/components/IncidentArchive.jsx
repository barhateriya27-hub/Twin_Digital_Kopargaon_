    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
              {t('incidentArchive.bannerTitle', 'Municipal Permanent Incident & Audit Archive')}
            </h2>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              {t('incidentArchive.bannerSubtitle', 'Immutable historical repository storing complaints, completion certificates, and audit logs')}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-lg text-xs font-semibold shrink-0">
          ...
          >
            {t('incidentArchive.tabComplaints', 'Complaints Archive')} ({complaints.length})
          </button>
          <button
          ...
          >
            {t('incidentArchive.tabReports', 'Completion Reports')} ({completedReports.length})
          </button>
          <button
          ...
          >
            {t('incidentArchive.tabAudit', 'System Audit Logs')} ({auditLogs.length})
          </button>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 text-xs">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder={t('incidentArchive.searchPlaceholder', 'Search by ID, keyword, citizen, officer...')}
        ...
        <div>
          ...
          >
            <option value="All">{t('incidentArchive.allWards', 'All Wards')}</option>
            {wards.map(w => (
        ...
        <div>
          <select
            ...
            className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-900 dark:text-slate-100 focus:outline-none"
          >
            <option value="All">{t('incidentArchive.allDepts', 'All Departments')}</option>
            {departments.map(d => (
        ...
        <div>
          ...
          >
            ...
            <option value="Escalated">{t('incidentArchive.statusEscalated', 'Escalated')}</option>
            <option value="Completed">{t('incidentArchive.statusCompleted', 'Completed')}</option>
          </select>
        </div>
      </div>
            ...
            <table className="w-full min-w-[650px] text-left border-collapse text-xs">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-950 text-slate-500 font-semibold border-b border-slate-200 dark:border-slate-800 uppercase tracking-wider text-[10px]">
                  <th className="p-3">{t('incidentArchive.thComplaintId', 'Complaint ID')}</th>
                  <th className="p-3">{t('incidentArchive.thCategoryTitle', 'Category & Title')}</th>
                  <th className="p-3">{t('incidentArchive.thWard', 'Ward')}</th>
                  <th className="p-3">{t('incidentArchive.thDept', 'Department')}</th>
                  <th className="p-3">{t('incidentArchive.thSla', 'SLA Status')}</th>
                  <th className="p-3">{t('incidentArchive.thDate', 'Submitted Date')}</th>
                  <th className="p-3 text-right">{t('incidentArchive.thAction', 'Action')}</th>
                </tr>
              </thead>
                ...
                {filteredComplaints.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="p-8 text-center text-slate-400">
                      {t('incidentArchive.noComplaints', 'No archived complaints match the specified filter criteria.')}
                    </td>
                  </tr>
                ) : (
                  filteredComplaints.map(item => (
                    <tr key={item.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
                      ...
                      <td className="p-3 font-medium text-slate-700 dark:text-slate-300">
                        {t('taxPortal.wardPrefix', 'Ward')} {item.ward}
                      </td>
                      ...
                      <td className="p-3 text-right">
                        ...
                        >
                          ...
                          <span>{t('incidentArchive.btnViewTicket', 'View Ticket')}</span>
                        </button>
                      </td>
                    </tr>
                  ))
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {completedReports.length === 0 ? (
            <div className="md:col-span-2 p-12 text-center bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 text-slate-400">
              ...
              <p className="text-xs font-medium">{t('incidentArchive.noReports', 'No completed work reports logged in archive yet')}</p>
            </div>
          ) : (
              ...
              <div key={c.id} className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-3">
                <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-2">
                  ...
                  <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300">
                    {t('incidentArchive.certGen', 'Official Certificate Generated')}
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between text-[11px] text-slate-500 pt-2 border-t border-slate-100 dark:border-slate-800">
                  ...
                  >
                    ...
                    <span>{t('incidentArchive.btnDownloadReport', 'Download Report')}</span>
                  </button>
                </div>
              </div>
            ))
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[650px] text-left border-collapse text-xs font-mono">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-950 text-slate-500 font-semibold border-b border-slate-200 dark:border-slate-800 uppercase tracking-wider text-[10px]">
                  <th className="p-3">{t('incidentArchive.thLogId', 'Log ID')}</th>
                  <th className="p-3">{t('incidentArchive.thTimestamp', 'Timestamp')}</th>
                  <th className="p-3">{t('incidentArchive.thActor', 'Actor / Role')}</th>
                  <th className="p-3">{t('incidentArchive.thIp', 'IP Address')}</th>
                  <th className="p-3">{t('incidentArchive.thSystemAction', 'System Action')}</th>
                  <th className="p-3">{t('incidentArchive.thTargetEntity', 'Target Entity')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {filteredAudit.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="p-8 text-center text-slate-400">
                      {t('incidentArchive.noAuditEntries', 'No audit entries matching filter criteria.')}
                    </td>
                  </tr>
                ) : (
